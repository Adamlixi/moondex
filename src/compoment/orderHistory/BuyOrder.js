...
import { getOrdersFromAO } from '../../util/utils';
...
class BuyOrder extends Component {
  ...
  async fetchData() {
    const orders = await getOrdersFromAO(DEX_ADDRESS, 'GetOrders', "1");
    const buyOrders = orders
      .filter((order) => order.direction !== 2)
      .map((order, index) => {
  ...
  render() {
    ...
    return (
      <Table dataSource={data}>
        <Column title="BUY ORDER" dataIndex="time" key="time" />
